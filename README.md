...
- [x] **Phase 4 — Chỉnh sửa nội dung (Edit)** ⭐ tính năng lõi/moat ✅ Iteration 1 + 2 + 3 hoàn tất (52/52 test xanh ngoài qpdf)
  ...
  - [x] **Iteration 4 — Sửa text TRONG Form XObject**: file gói nội dung vào form được "mở gói" ra cấp trang rồi sửa như thường, có **cổng an toàn so render** (lệch >0.5% pixel → từ chối + thông điệp rõ, bảo toàn file — file Canva phức tạp rơi vào nhóm này, kế hoạch hỗ trợ đầy đủ ở docs/12); CI chạy `cargo test` trước khi build
